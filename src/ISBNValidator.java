...
public class ISBNValidator {
    ...
    public boolean isValidISBN(String isbn)
    {
        ...
        if (prefix == 979 || prefix == 978) {
            String entire = isbn.replace("-", "");

            for (int i = 0; i < entire.length(); i++) {
                if (i % 2 == 0) {
                    sum += Integer.parseInt(entire.substring(i, i + 1));
                } else {
                    sum += 3 * Integer.parseInt(entire.substring(i, i + 1));
                }
            }
        }
        else{
    ...
    /**
     * Main method of class ISBNValidator
